...
export async function createMessage (
    recipeintId: string, data: MessageSchema): Promise<ActionResult<MessageDto>> {
    try {
      ...
      await pusherServer.trigger(createChatId(userId,recipeintId),'message:new',messageDto);
      await pusherServer.trigger(`private-${recipeintId}`,'message:new',messageDto);
...
export async function getMessageByContainer(container?:string | null,cursor?:string,limit=2){
    try {
        const userId=await getAuthUserId();
